...
class ChunkConverter extends GroupConverter {
    ...
    switch (vecType) {
      ...
      case Vec.T_NUM:
        if (OriginalType.DECIMAL.equals(parquetType.getOriginalType()))
          return new NullDecimalConverter();
        else
          return new NullNumberConverter();
      default:
        throw new UnsupportedOperationException("Unsupported type " + vecType);
    }
  }

  private static class NullNumberConverter extends PrimitiveConverter {
  ...
  private static class NullDecimalConverter extends PrimitiveConverter {
