...
def import_gzip_skipped_columns():
    ...
    try:
        bad = h2o.import_file(filePath, skipped_columns=list(range(airlineCSV.ncol)))  # skipped all
        sys.exit(1)
    ...
    try:
        bad = h2o.upload_file(filePath, skipped_columns=list(range(airlineCSV.ncol)))   # skipped all
        sys.exit(1)
    except Exception as ex:
        print(ex)
        pass
